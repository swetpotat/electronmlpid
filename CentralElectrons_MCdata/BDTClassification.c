...
int BDTClassification( TString myMethodList = "" ){
   ...
   TString fname = "./data/MC_SigBkgElectrons_2000000ev.root";
   if (gSystem->AccessPathName( fname ))  // file does not exist in local directory
      gSystem->Exec("curl -O http://root.cern.ch/files/tmva_class_example.root");
   TFile *input = TFile::Open( fname );
   std::cout << "--- BDTClassification       : Using input file: " << input->GetName() << std::endl;

   // Register the training and test trees
   TTree *signalTree     = (TTree*)input->Get("data");
   TTree *background     = (TTree*)input->Get("data");
   ...
   TMVA::Factory *factory = new TMVA::Factory( "BDTClassification", outputFile,
   ...
   std::cout << "YOU ARE HERE NOW" << std::endl;
   if (Use["BDTB"]) // Bagging
...
int main( int argc, char** argv )
{
   ...
   return BDTClassification(methodList);
}
